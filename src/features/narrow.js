...
export const createNarrows = createAsyncThunk('narrows/createNarrows', async (req) => {
	...
	if (!data.statusCode) {
		req.callback(true, 'Thêm ngành hẹp thành công');
		return data;
	}
	req.callback(false, data.message);
	return {};
});
...
export const updateNarow = createAsyncThunk('narrows/updateNarrows', async (req) => {
	...
	delete req.data._id;
	const { data } = await updateNarrows(id, req.data);
	if (!data.statusCode) {
		req.callback(true, 'Sửa ngành hẹp thành công');
		return data;
	}
	req.callback(false, data.message);
	return {};
